...
python scripts/run_paper_suite.py \
  --config "$CONFIG" \
  --stages all \
  2>&1 | tee logs/object_context_core_full.log
...
python scripts/analyze_object_context.py \
  ...
  --experiment-tag core_main_georsclip \
  2>&1 | tee logs/object_context_core_classwise.log

python scripts/analyze_paper_results.py \
  --config "$CONFIG" \
  2>&1 | tee logs/object_context_core_analysis.log

echo "Complete outputs are under outputs/results/object_context_core_v1/"
