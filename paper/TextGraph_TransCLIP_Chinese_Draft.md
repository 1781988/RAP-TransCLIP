...
本文采用聚焦设计：保持 RS-TransCLIP 的高斯混合模型、均匀类别先验、106 个提示模板平均文本原型及交替更新不变，只使用文本后验修正视觉图的边导通率。这样能够直接回答一个问题：文本模态能否作为图边界证据，减少视觉图在遥感相似场景之间的错误传播？

对于视觉近邻 \((i,j)\)，TextGraph-TransCLIP 计算两幅图像零样本类别后验之间的 Hellinger 亲和度。当两个节点均具有较高预测置信度且后验分布明显不一致时，对应视觉边被抑制；当任一节点预测不确定时，文本门控趋近于 1，保留原始视觉边。该设计只修改图权重，不引入新的类别先验或提示学习模块；文本门控是连续的，不进行不可逆硬边删除；语义亲和度与视觉亲和度的乘性融合可解释为跨模态边导通率。
...
TransCLIP 通过目标特征分布和文本先验联合更新无标签测试样本的预测 [5]。RS-TransCLIP 在遥感场景分类中加入图像亲和关系，并使用共享对角协方差的高斯混合模型 [6]。本文针对 RS-TransCLIP 目标函数中最直接的结构假设——固定视觉图同质性——进行单点改造。

### 2.3 图上的边界保持传播

传统图半监督学习通常利用特征相似度构建图，并通过拉普拉斯平滑传播标签。固定各向同性平滑会在异质边上造成类别边界模糊。边界保持图方法通常根据局部标签分布、预测差异或自适应导通率抑制跨类边。视觉语言模型提供了独立于视觉近邻的文本后验，因此可用于判断一条高视觉相似边是否具有一致语义。本文使用 Hellinger 亲和度而非硬伪标签一致性，保留后验分布中的类别不确定性。
...
式（9）具有两个边界性质：当任一节点接近均匀预测时，\(q_{ij}\rightarrow0\)，从而 \(g_{ij}\rightarrow1\)，图退化为原始视觉图；当两个节点均高置信但后验分布不一致时，\(q_{ij}\) 较大、\(a_{ij}\) 较小，对应边被显著削弱。
...
实验使用 AID [8]、EuroSAT [9]、MLRSNet [10]、OPTIMAL31 [11]、PatternNet [12]、RESISC45 [13]、RSC11 [14]、RSICB128、RSICB256 [15] 和 WHURS19 [16]。第一阶段固定 GeoRSCLIP ViT-L/14，以复用现有图像与文本特征，并与当前复现结果直接比较。第二阶段扩展至 CLIP、RemoteCLIP、GeoRSCLIP 和 SkyCLIP50 的 ViT-L/14；只有在方法稳定后才运行全部 11 个模型—架构组合。

比较方法包括：等权提示零样本 VLM；复现的 RS-TransCLIP；仅将视觉图改为互近邻图的结构基线；视觉图乘以 Hellinger 语义亲和度但不使用置信度回退；完整 TextGraph-TransCLIP。
...
从 RS-TransCLIP 的混淆矩阵中提取高频互混淆类别对，例如住宅密度类别、港口与河流、工业区与商业区。对每个类别对报告 RS-TransCLIP 类别对平均准确率、TextGraph-TransCLIP 类别对平均准确率、两类之间的视觉边数，以及 TextGraph 对跨类边总权重的削弱比例。类别对根据统一的 RS-TransCLIP 基线确定，并在报告中列出全部选择规则，避免只展示有利样例。
...
[8] G.-S. Xia et al., “AID: A benchmark data set for performance evaluation of aerial scene classification,” *IEEE Transactions on Geoscience and Remote Sensing*, vol. 55, no. 7, pp. 3965–3981, 2017.

[9] P. Helber et al., “Introducing EuroSAT: A novel dataset and deep learning benchmark for land use and land cover classification,” in *Proc. IGARSS*, 2018.

[10] X. Qi et al., “MLRSNet: A multi-label high spatial resolution remote sensing dataset for semantic scene understanding,” *ISPRS Journal of Photogrammetry and Remote Sensing*, vol. 169, pp. 337–350, 2020.

[11] Q. Wang et al., “Scene classification with recurrent attention of VHR remote sensing images,” *IEEE Transactions on Geoscience and Remote Sensing*, vol. 57, no. 2, pp. 1155–1167, 2019.

[12] W. Zhou et al., “PatternNet: A benchmark dataset for performance evaluation of remote sensing image retrieval,” *ISPRS Journal of Photogrammetry and Remote Sensing*, vol. 145, pp. 197–209, 2018.
...
[14] L. Zhao et al., “Feature significance-based multi-bag-of-visual-words model for remote sensing image scene classification,” *Journal of Applied Remote Sensing*, vol. 10, 2016.

[15] H. Li et al., “RSI-CB: A large-scale remote sensing image classification benchmark using crowdsourced data,” *Sensors*, vol. 20, no. 6, 2020.

[16] G.-S. Xia et al., “Structural high-resolution satellite image indexing,” *International Archives of the Photogrammetry, Remote Sensing and Spatial Information Sciences*, vol. 38, 2010.
